...
export default function EditStudentModal({ student, onSubmit, onClose, submitting }: Props) {
  ...
  const set = (key: keyof UpdateStudentForm, val: string | number) => setForm(p => ({ ...p, [key]: val }));
        ...
        <form onSubmit={handleSubmit} className="p-5 space-y-5">
          ...
          <div>
            ...
            <div className="grid grid-cols-3 gap-3">
              <div><label className="block text-xs font-medium text-slate-600 mb-1">Year Level</label>
                <select value={form.yearLevel} onChange={e => set('yearLevel', Number(e.target.value))} className={inputCls}>
                  <option value={0}>Freshman</option><option value={1}>Sophomore</option><option value={2}>Junior</option><option value={3}>Senior</option>
                </select>
              </div>
              ...
              <div><label className="block text-xs font-medium text-slate-600 mb-1">Advisor</label>
                <select value={form.academicAdvisorId ?? ''} onChange={e => set('academicAdvisorId', e.target.value ? Number(e.target.value) : undefined as unknown as number)} className={inputCls}>
                  <option value="">— Select —</option>
          ...
          <div>
            ...
            <div className="grid grid-cols-3 gap-3">
              <div><label className="block text-xs font-medium text-slate-600 mb-1">Gender</label>
                <select value={form.gender} onChange={e => set('gender', Number(e.target.value))} className={inputCls}><option value={0}>Male</option><option value={1}>Female</option></select>
              </div>
          ...
          {/* Family / Previous */}
          <div>
            <h3 className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-3">Family & Background</h3>
            <div className="grid grid-cols-3 gap-3">
