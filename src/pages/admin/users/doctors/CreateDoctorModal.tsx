...
export default function CreateDoctorModal({ onSubmit, onClose, submitting }: Props) {
  const [form, setForm] = useState<CreateDoctorForm>({
        ...
        <form onSubmit={handleSubmit} className="p-5 space-y-4">
          ...
          <div className="grid grid-cols-3 gap-3">
            <div><label className="block text-xs font-medium text-slate-600 mb-1">Title *</label>
              <select value={form.title} onChange={e => setForm(p => ({ ...p, title: Number(e.target.value) }))} className={inputCls}>
            ...
            <div><label className="block text-xs font-medium text-slate-600 mb-1">Department *</label>
              <select value={form.departmentId} onChange={e => setForm(p => ({ ...p, departmentId: Number(e.target.value) }))} className={inputCls}>
          ...
          <div className="flex items-center justify-end gap-2 pt-2">
            ...
            <button type="submit" disabled={submitting || !form.name || !form.email || !form.phoneNumber || !form.nationalId || !form.departmentId}
