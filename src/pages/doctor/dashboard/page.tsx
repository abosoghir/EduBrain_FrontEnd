...
export default function DoctorDashboard() {
  ...
  useEffect(() => {
    api.get<ApiResponse<DoctorDashboardData>>('/api/doctor/dashboard')
      .then((res) => {
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        {loading ? (
          ...
          [
            ...
            { label: 'Students', value: data?.totalStudents ?? 0, icon: 'ri-user-line', color: 'text-emerald-600 bg-emerald-50' },
            { label: 'Pending', value: data?.pendingQuizzes ?? 0, icon: 'ri-draft-line', color: 'text-amber-600 bg-amber-50' },
            { label: 'Events Today', value: data?.eventsSentToday ?? 0, icon: 'ri-send-plane-line', color: 'text-slate-700 bg-slate-100' },
          ].map((s) => (
            <button
              key={s.label}
              type="button"
          ...
          {loading ? (
            ...
            <div className="space-y-3">
              {data.todaySchedule.map((slot) => (
                <div key={slot.courseScheduleId} className="flex items-center gap-3 p-3 rounded-lg bg-gray-50 hover:bg-gray-100 transition-colors">
          ...
          {loading ? (
            ...
            <div className="space-y-3">
              {data.recentAnnouncements.map((a) => (
                <div key={a.notificationId} className="flex items-start gap-3 p-3 rounded-lg bg-gray-50 hover:bg-gray-100 transition-colors">
                  <div className="w-8 h-8 rounded-lg bg-violet-50 flex items-center justify-center shrink-0">
                    <i className="ri-megaphone-line text-violet-500 text-sm" />
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium text-slate-700">{a.title}</p>
                    <p className="text-[10px] text-slate-400">{a.recipientsCount} recipients · {new Date(a.sentDate).toLocaleDateString()}</p>
                  </div>
                </div>
              ))}
            </div>
